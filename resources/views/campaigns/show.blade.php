...
<div class="px-6 py-6 bg-gray-50 min-h-screen">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-5">
        ...
        {!! $stat('Budget Approval', $fd($campaign->budget_approval_date)) !!}
                            ...
                            @if($rows->count())
                                ...
                                @foreach($rows as $p)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="py-2.5 px-3 text-right whitespace-nowrap">€{{ number_format((float)$p->price, 0) }}</td>
                                        <td class="py-2.5 px-3 text-gray-500 whitespace-nowrap">{{ $fd($p->date_to_copywriter) }}</td>
                                        <td class="py-2.5 px-3 text-gray-500 whitespace-nowrap">{{ $fd($p->date_from_copywriter) }}</td>
                                        <td class="py-2.5 px-3 text-gray-500 whitespace-nowrap">{{ $fd($p->date_to_blog) }}</td>
                                        <td class="py-2.5 px-3">
                                            @if($p->live_url)
                                                <a href="{{ $p->live_url }}" target="_blank" class="text-green-600 hover:underline text-xs">{{ \Illuminate\Support\Str::of($p->live_url)->replace(['https://','http://'],'')->limit(28) }}</a>
                                            @else <span class="text-gray-300">—</span> @endif
                                        </td>
                                        <td class="py-2.5 px-3 text-gray-500 whitespace-nowrap">{{ $fd($p->live_date) }}</td>
