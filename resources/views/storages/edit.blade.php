    ...
    <div class="form-card max-w-6xl">
        ...
        <form method="POST" action="{{ route('storages.update', $storage->id) }}" class="space-y-4">
            ...
            <div class="grid grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="number" step="1" min="0" name="link_builder_amount"
                           value="{{ old('link_builder_amount', $storage->link_builder_amount) }}"
                           class="w-full border border-gray-300 rounded px-2 py-1 focus:ring-green-500 focus:border-green-500">
            ...
            @php
                ...
                $auto = ['total_cost','total_revenues','profit'];
            @endphp
            <div class="grid grid-cols-4 gap-4">
                @foreach($priceFields as $field => $label)
                    @php $autoCalc = in_array($field,$auto); @endphp
                    <div>
                        <label class="block {{ $autoCalc ? 'text-gray-600' : 'text-gray-700' }} font-medium mb-1">
                            {{ $label }}{!! $autoCalc ? ' <span class="text-xs text-gray-400">(auto)</span>' : '' !!}
                        </label>
                        <input
                            type="number"
                            step="0.01"
